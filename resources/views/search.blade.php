        ...
        @if (request('type') === 'images')
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                @foreach ($data as $item)
                    <div class="group">
                        <div
                            class="overflow-hidden rounded-lg bg-gray-100 dark:bg-gray-800 shadow-sm hover:shadow-md transition-shadow duration-200">
                            <img class="w-full h-40 object-cover object-center group-hover:opacity-90 transition-opacity duration-200"
                                src="http://books.toscrape.com/{{ $item['image'] }}" alt="{{ $item['title'] }}">
                        </div>
                        <div class="mt-2">
                            <h3 class="text-sm text-gray-700 dark:text-gray-300 line-clamp-1 transition-colors">
                                {{ $item['title'] }}</h3>
                            <p class="text-xs text-gray-500 dark:text-gray-400 transition-colors">{{ $item['price'] }}
                            </p>
                        </div>
                    </div>
                @endforeach
                    ...
                    <div
                        ...
                        <div class="p-4">
                            <h3
                                ...
                                {{ $item['title'] }}</h3>
                            <p class="mt-2 text-lg font-bold text-green-600 dark:text-green-400 transition-colors">
                                {{ $item['price'] }}</p>
                ...
                @foreach ($data as $item)
                    <div class="max-w-3xl">
                        <div class="flex flex-col">
                            <div
                                class="flex items-center text-xs text-gray-600 dark:text-gray-400 mb-1 transition-colors">
                                <span class="truncate">books.toscrape.com › {{ Str::slug($item['title']) }}</span>
                            </div>
                            <h3
                                class="text-xl text-blue-800 dark:text-blue-400 hover:underline font-medium mb-1 transition-colors">
                                <a href="#">{{ $item['title'] }}</a>
                            </h3>
                            <div class="flex items-start">
                                <div class="hidden sm:block mr-4">
                                    <img class="w-20 h-24 object-cover rounded border border-gray-200 dark:border-gray-700"
                                        src="http://books.toscrape.com/{{ $item['image'] }}"
                                        alt="{{ $item['title'] }}">
                                </div>
                                <div>
                                    <p class="text-sm text-gray-700 dark:text-gray-300 mb-1 transition-colors">
                                        {{ Str::limit('This is a book about ' . $item['title'] . ". It's a fascinating read that explores various themes and ideas. The author has crafted a compelling narrative that keeps readers engaged from start to finish.", 150) }}
                                    </p>
                                    <div class="flex items-center mt-1">
                                        <span
                                            class="text-sm font-medium text-green-700 dark:text-green-500 transition-colors">{{ $item['price'] }}</span>
                                        <span class="mx-2 text-gray-300 dark:text-gray-600 transition-colors">|</span>
                                        <div class="flex text-yellow-400">
                                            @for ($i = 0; $i < 5; $i++)
                                                @if ($i < rand(3, 5))
